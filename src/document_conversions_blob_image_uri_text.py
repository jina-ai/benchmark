...
def test_document_convert_image_uri_to_blob(num_docs, json_writer):
    ...
    def _input_docs():
        ...
        return (), dict(
            docs=[Document(uri=image_dir) for _ in range(num_docs)]
        )
...
def test_document_convert_buffer_to_blob(num_docs, json_writer):
        ...
        def _input_docs():
            return (), dict(
                docs=[Document(content=np.random.random((85, 152, 3))) for _ in range(num_docs)]
            )
...
def test_document_convert_image_blob_to_uri(num_docs, json_writer):
        ...
        def _input_docs():
            return (), dict(
                docs=[Document(content=np.random.randint(0, 255, 32 * 28)) for _ in range(num_docs)]
            )
